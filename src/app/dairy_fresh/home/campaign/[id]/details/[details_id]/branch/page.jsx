...
export default function Branch() {
  ...
  const fetchBranch = async (page = 1) => {
    setLoading(true);

    try {
      const res = await api.get(
        `/api/branch?page=${page}&&search=${searchTerm}&&campaign_details_id=${details_id}`,
        {
          headers: {
  ...
  const handleAssign = async (branch_id) => {
    
    try {
      const res = await api.put(
        `/api/branch/${branch_id}/campaign_details`,
        { campaign_details_id: details_id },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/branch/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    ...
    ${openMore ? "max-h-40 opacity-100 p-5 " : "max-h-0 opacity-0 p-0"}`}
        >
          ...
          <Link
            href={`/dairy_fresh/home/branch/new`}
