...
export default function PaymentType() {
  ...
  const fetchPaymentTypes = async (page = 1) => {
    ...
    try {
      const res = await api.get(
        `/api/payment_info/type?page=${page} && 
        search=${searchTerm}`,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/payment_info/type/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleActivation = async (id) => {
    ...
    try {
      const res = await api.put(
        `/api/payment_info/type/${id}/activation`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    ...
    ${openMore ? "max-h-40 opacity-100 p-5 " : "max-h-0 opacity-0 p-0"}`}
        >
          ...
          <Link
            href={`/dairy_fresh/home/payment_info/type/new`}
            ...
            {payment_types.map((payment_type) => (
              <tr key={payment_type.id} className="border-b border-gray-200">
                ...
                <td className="flex gap-2 p-4 border-b border-gray-200">
                  <Link
                    href={`/dairy_fresh/home/payment_info/type/edit/${payment_type.id}`}
                    className="text-blue-600 hover:underline"
                  >
                    Edit
                  </Link>
