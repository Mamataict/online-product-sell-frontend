...
export default function PaymentInfo() {
  ...
  const fetchPaymentTypes = async (page = 1) => {
    setLoading(true);

    try {
      const res = await api.get(
        `/api/payment_info?page=${page} && 
        search=${searchTerm}`,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/payment_info/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleActivation = async (id) => {
    ...
    try {
      const res = await api.put(
        `/api/payment_info/${id}/activation`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="max-w-3xl p-4 bg-white rounded-md shadow-md">
      <div>
        <div className="flex">
          ...
          {hasPermission(["payment_type.store"]) && (
            <FontAwesomeIcon
              icon={faPlus}
    ...
    ${openMore ? "max-h-40 opacity-100 p-5 " : "max-h-0 opacity-0 p-0"}`}
        >
          ...
          <Link
            href={`/dairy_fresh/home/payment_info/new`}
        ...
        <table className="table-auto w-full text-left">
          <thead>
            <tr>
              ...
              <th className="p-4 border-b border-gray-200">Payment Type</th>
          ...
          <tbody>
            {payment_types.map((payment_type) => (
              <tr key={payment_type.id} className="border-b border-gray-200">
                <td className="p-4 border-b border-gray-200">
                  {payment_type.name}
                </td>
                <td className="p-4 border-b border-gray-200">
                  {payment_type?.payment_type?.name || "N/A"}
                </td>
                ...
                <td className="p-4 border-b border-gray-200">
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      handleActivation(payment_type.id);
                    }}
                    className={`cursor-pointer hover:underline ${
                      payment_type.is_active ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {payment_type.is_active ? "Active" : "Inactive"}
                  </a>
                </td>
                <td className="flex gap-2 p-4 border-b border-gray-200">
                  <Link
                    href={`/dairy_fresh/home/payment_info/edit/${payment_type.id}`}
                    className="text-blue-600 hover:underline"
                  >
                    Edit
                  </Link>
