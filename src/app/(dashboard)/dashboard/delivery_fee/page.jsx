...
export default function DeliveryFee() {
  const [delivery_fees, setDeliveryFees] = useState([]);
  ...
  const fetchDeliveryFees = async (page = 1) => {
    setLoading(true);

    try {
      const res = await api.get(
        `/api/delivery_fee?page=${page} && 
        search=${searchTerm}`,
        {
          headers: {
      ...
      setDeliveryFees(res.data.data.data);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/delivery_fee/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleActivation = async (id) => {
    ...
    try {
      const res = await api.put(
        `/api/delivery_fee/${id}/activation`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="max-w-3xl p-4 bg-white rounded-md shadow-md">
      <div>
        <div className="flex">
          ...
          <PermissionGuard permission={"product_category.store"}>
    ...
    ${openMore ? "max-h-40 opacity-100 p-5 " : "max-h-0 opacity-0 p-0"}`}
        >
          ...
          <Link
            href={`/dashboard/delivery_fee/new`}
      ...
      <div className="overflow-x-auto">
        <table className="table-auto w-full text-left">
          ...
          <tbody>
            {delivery_fees.map((delivery_fee) => (
              <tr
                key={delivery_fee.id}
              ...
              >
                ...
                <td className="p-4 border-b border-gray-200">
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      handleActivation(delivery_fee.id);
                    }}
                  ...
                  >
                    {delivery_fee.is_active ? "Active" : "Inactive"}
                  </a>
                </td>
                <td className="flex gap-2 p-4 border-b border-gray-200">
                  <Link
                    href={`/dashboard/delivery_fee/edit/${delivery_fee.id}`}
                    className="text-blue-600 hover:underline"
                  >
                    Edit
                  </Link>
