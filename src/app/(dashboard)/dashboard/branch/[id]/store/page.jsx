...
import List from "@/components/branch/store/List";
...
export default function BranchStores() {
  ...
  const fetchBranchStores = async (page = 1) => {
    try {
      const res = await api.get(
        `/api/branch/store?page=${page} && 
        search=${searchTerm} && branch_id=${id}`,
        {
          headers: {
      ...
      setBranchStores(res.data.data.data);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/branch/store/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleActivation = async (id) => {
    ...
    try {
      const res = await api.put(
        `/api/branch/store/${id}/activation`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="max-w-3xl p-4 bg-white rounded-md shadow-md">
      <div>
        <div className="flex">
          ...
          <PermissionGuard permission={"branch.store.store"}>
            <FontAwesomeIcon
              icon={faPlus}
    ...
    ${openMore ? "max-h-40 opacity-100 p-5 " : "max-h-0 opacity-0 p-0"}`}
        >
          ...
          <Link
            href={`/dashboard/branch/store/new`}
