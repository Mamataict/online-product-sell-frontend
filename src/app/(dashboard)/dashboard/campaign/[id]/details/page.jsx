...
import List from "@/components/campaign/details/List";
import PermissionGuard from "@/components/dashboard/PermissionGuard";

export default function BranchStores() {
  ...
  const fetchCampaignDetails = async (page = 1) => {
    try {
      const res = await api.get(
        `/api/campaign/details?page=${page} && 
        search=${searchTerm} && campaign_id=${id}`,
        {
          headers: {
      ...
      setcampaignDetails(res.data.data.data);
  ...
  useEffect(() => {
    if (currentPage >= 1 && currentPage <= lastPage) {
      fetchCampaignDetails(currentPage);
    }
  }, [currentPage]);

  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      fetchCampaignDetails(1);
    }, 300);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/api/campaign/details/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleActivation = async (id) => {
    ...
    try {
      const res = await api.put(
        `/api/campaign/details/${id}/activation`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="max-w-3xl p-4 bg-white rounded-md shadow-md">
      <div>
        <div className="flex">
          ...
          <PermissionGuard permission={"campaign.details.store"}>
    ...
    ${openMore ? "max-h-40 opacity-100 p-5 " : "max-h-0 opacity-0 p-0"}`}
        >
          ...
          <Link
            href={`/dashboard/campaign/${id}/details/new`}
